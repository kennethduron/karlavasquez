...
Los despliegues se controlan mediante Vercel CLI después de comprobar que `HEAD` coincide con `origin/feat/knv-public-site-phase-2` y que GitHub Actions terminó correctamente. Esta estrategia evita desplegar `main` o una fase histórica mientras no exista una política de ramas aprobada. No se usa `--force` ni se hace merge para asignar el alias.
